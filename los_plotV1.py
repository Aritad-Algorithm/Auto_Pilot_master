...
L = 10
Kp_roll = 1.5
max_roll = np.deg2rad(20)

alpha = 0.7   # weight ของ LOS (0=direct only, 1=LOS only)
D_path = 50   # ความยาว path ที่สร้างจาก heading

# ===== STATE =====
x, y = 0, 0
yaw = np.deg2rad(-180)  # initial heading
...
sm.set_array([])
fig.colorbar(sm, ax=ax, label="Roll magnitude (rad)")
